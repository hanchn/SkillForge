...
- **CMS 产品架构师**（`cms-product-architecture`，v1.0.5）
...
- **CRM 产品架构师**（`crm-product-architecture`，v1.0.6）
  - 路径：`互联网研发/产品/系统产品架构/CRM/crm-product-architecture/`
  - 分类：互联网研发 / 产品 / 系统产品架构 / CRM
  - 用途：围绕统一客户、身份解析、同意偏好、线索商机、互动服务、分群旅程和生命周期运营设计可审计的 CRM 产品架构。适用于跨渠道客户中心与客户 360、营销、销售与客售前后 CRM、线索、商机、服务工单和生命周期运营、CRM 重构、迁移、主数据与权限治理，交付CRM 业务能力与角色权限地图、客户身份、账户、联系人、同意和主数据模型、线索、商机、互动、工单、分群和旅程状态机、CRM 与 CDP/OMS/CMS/客服/营销/数据平台集成及迁移方案。
- **IMS 产品架构师**（`ims-product-architecture`，v1.0.5）
...
- **OFS 产品架构师**（`ofs-product-architecture`，v1.0.5）
...
- **OMS 产品架构师**（`oms-product-architecture`，v1.0.5）
...
- **PLM 产品架构师**（`plm-product-architecture`，v1.0.5）
...
- **TMS 产品架构师**（`tms-product-architecture`，v1.0.5）
...
- **WMS 产品架构师**（`wms-product-architecture`，v1.0.5）
...
- **跨系统产品架构师**（`commerce-systems-product-architecture`，v1.1.5）
...
- **UI 交互设计与实现专家**（`ui-interaction-designer`，v1.0.5）
...
- **前端功能实现工程师**（`frontend-feature-implementer`，v1.0.4）
...
- **前端多语言查验师**（`frontend-localization-verifier`，v1.0.4）
...
- **前端性能分析师**（`frontend-performance-analyzer`，v1.0.4）
...
- **Web 性能与无障碍审计师**（`web-performance-accessibility-auditor`，v1.0.4）
...
- **前端架构师**（`frontend-architecture-designer`，v1.0.6）
...
- **React 资深专家**（`react-senior-expert`，v1.0.6）
...
- **Vue3 资深专家**（`vue3-senior-expert`，v1.0.6）
...
- **API 契约设计师**（`api-contract-designer`，v1.0.4）
  - 路径：`互联网研发/后端/api-contract-designer/`
  - 分类：互联网研发 / 后端 / API设计
  - 用途：设计一致、可演进、可测试并能直接评审的 HTTP 或事件 API 契约
- **数据库变更迁移规划师**（`database-migration-planner`，v1.0.4）
...
- **后端服务架构师**（`backend-service-architect`，v1.0.4）
...
- **后端架构师**（`backend-architecture-designer`，v1.0.6）
...
- **ASP.NET Core 后端资深专家**（`aspnet-core-backend-architecture`，v1.0.6）
...
- **FastAPI 后端资深专家**（`fastapi-backend-architecture`，v1.0.6）
...
- **Gin 后端资深专家**（`gin-backend-architecture`，v1.0.6）
...
- **Laravel 后端资深专家**（`laravel-backend-architecture`，v1.0.6）
...
- **NestJS 后端资深专家**（`nestjs-backend-architecture`，v1.0.6）
...
- **Spring Boot 后端资深专家**（`spring-boot-backend-architecture`，v1.0.6）
...
- **生产故障诊断师**（`production-incident-diagnostician`，v1.0.4）
...
- **C# 后端资深专家**（`csharp-backend-architecture`，v1.0.6）
...
- **Go 后端资深专家**（`go-backend-architecture`，v1.0.6）
...
- **Java 后端资深专家**（`java-backend-architecture`，v1.0.6）
...
- **Node.js 后端资深专家**（`nodejs-backend-architecture`，v1.0.6）
...
- **PHP 后端资深专家**（`php-backend-architecture`，v1.0.6）
...
- **Python 后端资深专家**（`python-backend-architecture`，v1.0.6）
...
- **Rust 后端资深专家**（`rust-backend-architecture`，v1.0.6）
...
- **测试策略设计师**（`test-strategy-designer`，v1.0.4）
  - 路径：`互联网研发/测试/test-strategy-designer/`
  - 分类：互联网研发 / 测试 / 测试策略
  - 用途：根据变更影响和风险设计分层覆盖、发布门槛与生产验证方案
- **全栈开发资深专家**（`fullstack-feature-delivery`，v1.0.6）
...
- **全栈开发资深专家周报**（`fullstack-developer-weekly-report`，v1.0.5）
...
- **全栈测试资深专家**（`fullstack-quality-engineer`，v1.0.6）
...
- **全栈测试资深专家周报**（`fullstack-qa-weekly-report`，v1.0.5）
...
- **人事招聘资深经理**（`senior-hr-recruiting-manager`，v1.0.6）
...
- **人事招聘资深经理周报**（`senior-hr-recruiting-manager-weekly-report`，v1.0.7）
...
- **候选人寻访运营专员**（`candidate-sourcing-operator`，v1.0.5）
...
- **人事招聘通用执行专家**（`hr-recruitment-generalist`，v1.0.5）
...
- **绩效与员工关系管理专员**（`performance-employee-relations-manager`，v1.0.5）
...
- **企业员工公告撰写专家**（`employee-announcement-writer`，v1.0.5）
...
- **员工生命周期与档案管理员**（`employee-lifecycle-records-manager`，v1.0.5）
...
- **员工培训组织与运营经理**（`employee-training-organization-manager`，v1.0.5）
...
- **岗位说明书与招聘 JD 生成专家**（`job-description-generator`，v1.0.6）
  - 路径：`人事招聘/岗位与JD/job-description-generator/`
  - 分类：人事招聘 / 岗位与JD
  - 用途：先把业务岗位翻译成供 HR 学习和确认的详细岗位介绍，完成指定市场近期岗位与薪酬调研，再生成招聘预问问题、内部岗位说明书、候选人版 JD 与可复用空白 JD 模板，避免因岗位理解和市场判断偏差招错人。适用于HR 不熟悉技术、运营或专业岗位时的招聘需求培训、新岗位近几个月招聘需求、薪资分布和人才市场调研、新增或重写招聘 JD、岗位职责模糊、要求堆砌或招聘双方理解不一致、多平台、多语言或跨区域岗位发布，交付HR 岗位培训与详细岗位介绍、岗位市场与近期薪酬分布调研报告、招聘需求澄清与建议预问问题、内部岗位说明书、候选人版招聘 JD、可复制的 JD 空白模板、能力证据、面试评估和发布检查表。
- **录用与入职管理专员**（`offer-onboarding-manager`，v1.0.5）
...
- **人力编制与招聘规划师**（`workforce-recruiting-planner`，v1.0.5）
...
- **本地简历筛选与合规评估专家**（`local-resume-screening-compliance-specialist`，v1.0.0）
  - 路径：`人事招聘/简历筛选/local-resume-screening-compliance-specialist/`
  - 分类：人事招聘 / 简历筛选
  - 用途：以已确认 JD 和岗位相关证据标准读取授权本地简历，生成可解释匹配、缺口、核验问题、异常风险和合规提示，供 HR 人工复核。适用于给定 JD 后批量筛选本地 PDF/DOCX/TXT/MD 简历、候选人初筛、分层和面试问题准备、筛选规则、异常原因和公平合规审计，交付JD 能力与证据评分规则、逐候选人匹配、缺口、风险和核验问题卡、人工复核分层、异常队列和合规审计报告。
- **结构化面试与评估设计师**（`structured-interview-designer`，v1.0.5）
...
- **仓储库存资深经理**（`senior-warehouse-inventory-manager`，v1.0.6）
...
- **仓储库存资深经理周报**（`senior-warehouse-inventory-manager-weekly-report`，v1.0.6）
...
- **供应链资深经理**（`senior-supply-chain-manager`，v1.0.6）
...
- **供应链资深经理周报**（`senior-supply-chain-manager-weekly-report`，v1.0.6）
...
- **仓容、现场安全与连续性经理**（`warehouse-safety-capacity-manager`，v1.0.5）
...
- **仓库入库作业经理**（`warehouse-inbound-operations-manager`，v1.0.5）
...
- **仓库出库履约经理**（`warehouse-outbound-fulfillment-manager`，v1.0.5）
...
- **电商履约物流规划师**（`ecommerce-fulfillment-logistics-planner`，v1.0.4）
...
- **多仓多渠道库存分配规划师**（`inventory-allocation-planner`，v1.0.5）
...
- **库存控制与循环盘点经理**（`inventory-control-cycle-count-manager`，v1.0.5）
...
- **库存补货与分级告警规划师**（`inventory-replenishment-planner`，v2.0.4）
  - 路径：`仓储库存/库存计划/inventory-replenishment-planner/`
  - 分类：仓储库存 / 库存计划
  - 用途：端到端完成多渠道库存补货与分级告警，包括需求和交期、安全库存、告警阈值、异常队列、认领 SLA、采购交接、关闭复盘、渠道分配、现金和仓储约束
- **退货与逆向物流经理**（`returns-reverse-logistics-manager`，v1.0.5）
...
- **AIGC 创意制作专家**（`ai-image-video-production-manager`，v1.0.6）
...
- **智能剪辑工作流专家**（`intelligent-video-editing-specialist`，v1.0.6）
...
- **创意素材智能打标专家**（`creative-asset-intelligent-tagging-specialist`，v1.1.3）
  - 路径：`创拍视觉/AI创意制作/智能打标/creative-asset-intelligent-tagging-specialist/`
  - 分类：创拍视觉 / AI创意制作 / 智能打标
  - 用途：为贵司图片、视频和广告素材库设计并运行从关键词/标签库确认、素材输入、模型推理、标签建议、人审反馈、CMS/DAM 回写到持续评估的可治理 AI 打标闭环。适用于存量图片视频批量打标、新素材入库自动标签建议、接入贵司关键词库或标签库并按确认策略优先匹配、AI 打标工具规划、标签补全、纠错、搜索和效果归因治理，交付关键词/标签库接入与优先级确认记录、AI 打标能力、用户流程与标签体系、模型、置信度、人审决策和批量回写方案、金标评估、监控、错误集、版本、回滚和演进报告。
- **AI 创意真实性与合规审查师**（`ai-creative-authenticity-compliance-reviewer`，v1.0.6）
...
- **虚拟模特内容制作专家**（`virtual-model-content-producer`，v1.0.6）
...
- **UGC 与达人内容制作经理**（`ugc-content-production-manager`，v1.0.6）
...
- **跨境广告创意策略师**（`creative-strategy-producer`，v1.0.6）
...
- **创拍视觉资深经理**（`senior-creative-production-manager`，v1.0.7）
...
- **创拍视觉资深经理周报**（`senior-creative-production-manager-weekly-report`，v1.0.7）
...
- **剪辑后期与素材交付经理**（`post-production-delivery-manager`，v1.0.6）
...
- **商品摄影导演**（`product-photography-director`，v1.0.6）
...
- **商品图片与视频拍摄制作经理**（`product-photo-video-production-manager`，v1.0.6）
...
- **拍摄制片与现场统筹**（`shooting-production-coordinator`，v1.0.6）
...
- **跨境短视频制作导演**（`short-form-video-producer`，v1.0.6）
...
- **创意素材运营经理**（`creative-asset-operations-manager`，v1.0.6）
...
- **企业通用周报**（`general-company-weekly-report`，v1.0.0）
  - 路径：`基础工具/汇报复盘/general-company-weekly-report/`
  - 分类：基础工具 / 汇报复盘
  - 用途：把一个团队或跨部门周期内的真实数据、交付、风险、决策和下周承诺整理成可核验周报，同时保留业务角色专属口径。适用于缺少专属角色 Skill 的团队周报、跨部门项目周报、管理层通用进展与风险汇报，交付结论先行周报、数据证据与状态对照表、风险、决策、依赖和下周验收计划。
- **复盘与行动闭环主持专家**（`retrospective-action-closure-facilitator`，v1.0.0）
  - 路径：`基础工具/汇报复盘/retrospective-action-closure-facilitator/`
  - 分类：基础工具 / 汇报复盘
  - 用途：基于事件、项目、活动或经营周期证据组织无责复盘，区分时间线、事实、促成条件和根因，并追踪行动到效果验证。适用于事故与异常复盘、项目、发布或活动复盘、月度季度经营复盘与重复问题治理，交付证据化复盘报告、促成因素、根因和系统性改进、行动、owner、验证、关闭与复发监控台账。
- **多平台消息通知与分级告警编排专家**（`message-alert-orchestration-specialist`，v1.0.0）
  - 路径：`基础工具/消息与告警/message-alert-orchestration-specialist/`
  - 分类：基础工具 / 消息与告警
  - 用途：把业务、系统和运营事件编排为可去重、可升级、可回执、可关闭的多平台通知告警，并安全适配飞书、钉钉、企业微信及贵司企业 QQ 网关。适用于库存、订单、财务、系统和运营指标告警、飞书、钉钉、企业微信机器人或应用接入、企业 QQ/内部消息网关适配与多平台容灾，交付事件与严重度模型、平台连接器、路由、模板和权限方案、发送预览、回执、升级、关闭和复盘记录。
### 客售前后

- **客户咨询分析专家**（`customer-consultation-analysis-specialist`，v1.0.0）
  - 路径：`客售前后/咨询分析/customer-consultation-analysis-specialist/`
  - 分类：客售前后 / 咨询分析
  - 用途：把授权的售前、售中、售后咨询与订单、商品、履约、退款和评价事实关联，识别客户意图、知识缺口、转化障碍、服务异常和可验证改进机会。适用于客服会话与咨询主题分析、高频问题、未解决咨询和转化障碍诊断、咨询、订单、售后、评价与 VOC 联合分析，交付咨询数据质量与口径报告、意图主题、问题严重度和根因分析、知识、商品、流程、培训和系统改进闭环。
- **客售前后资深经理**（`senior-customer-service-presales-manager`，v1.0.7）
  - 路径：`客售前后/客服角色/客售前后资深经理/senior-customer-service-presales-manager/`
  - 分类：客售前后 / 客服角色 / 客售前后资深经理
  - 用途：统筹跨渠道售前咨询、订单中服务、售后处理、商品推荐、多语言沟通、知识库、质检培训和跨部门交接。适用于由客售前后资深经理接管复杂经营任务、制定季度或月度计划、跨专项协同、经营诊断和复盘，交付角色经营目标与责任边界、专项 Skill 编排与决策、经营节奏、风险和团队行动计划。
- **客售前后资深经理周报**（`senior-customer-service-presales-manager-weekly-report`，v1.0.7）
  - 路径：`客售前后/客服角色/客售前后资深经理/senior-customer-service-presales-manager-weekly-report/`
  - 分类：客售前后 / 客服角色 / 客售前后资深经理
  - 用途：基于真实经营证据生成客售前后资深经理视角的结论先行周报，并跟踪目标、驱动、风险、决策和责任闭环。适用于客售前后资深经理本周经营复盘、本周与上周或目标对比、管理层同步和跨团队行动跟踪，交付角色经营周报、指标变化和驱动解释、风险、决策与下周行动表。
- **跨境客售前后运营专家**（`customer-service-voice-manager`，v2.0.6）
  - 路径：`客售前后/客服运营/customer-service-voice-manager/`
  - 分类：客售前后 / 客服运营
  - 用途：端到端完成购买前咨询、商品推荐、订单中服务、售后处理、多语言沟通、知识沉淀、质检培训和跨部门交接。适用于邮件、聊天和平台消息的售前售中售后接待、商品推荐、订单查询、异议、退款退货与多语言沟通、客服知识库、投诉升级、线索交接和质量改进，交付客售前后答复与处理方案、客服 SOP、FAQ、话术、权限和升级矩阵、咨询、售后、质检、培训与 VOC 改进报告。
### 市场采购

- **供应商寻源与质量管理器**（`supplier-sourcing-quality-manager`，v1.0.4）
...
- **供应商谈判与成本经理**（`supplier-negotiation-cost-manager`，v1.0.5）
...
- **市场需求与商品机会分析师**（`market-demand-opportunity-analyst`，v1.0.5）
...
- **采购样品验证经理**（`sample-evaluation-sourcing-manager`，v1.0.5）
...
- **跨境电商选品研究**（`ecommerce-product-research`，v1.0.4）
  - 路径：`市场采购/选品/ecommerce-product-research/`
  - 分类：市场采购 / 选品
  - 用途：从需求、竞争、利润、供应链、合规和渠道适配评估跨境产品机会
- **采购订单与交付经理**（`purchase-order-delivery-manager`，v1.0.5）
...
- **市场采购资深经理**（`senior-market-procurement-manager`，v1.0.6）
...
- **市场采购资深经理周报**（`senior-market-procurement-manager-weekly-report`，v1.0.6）
...
- **选品与商品资深经理**（`senior-merchandising-manager`，v1.0.6）
...
- **选品与商品资深经理周报**（`senior-merchandising-manager-weekly-report`，v1.0.6）
...
- **采购风险与供应韧性经理**（`procurement-risk-compliance-manager`，v1.0.5）
...
- **广告效果分析师**（`advertising-performance-analyst`，v1.0.5）
...
- **业务指标治理师**（`business-metrics-governance`，v1.0.5）
...
- **电商转化漏斗分析师**（`conversion-funnel-analyst`，v1.0.5）
...
- **客户 Cohort 与 LTV 分析师**（`customer-cohort-ltv-analyst`，v1.0.5）
...
- **经营分析与周月报专家**（`executive-business-review`，v1.0.5）
...
- **经营预测与情景规划师**（`forecast-scenario-planner`，v1.0.5）
...
- **库存与供应链分析师**（`inventory-supply-chain-analyst`，v1.0.5）
...
- **跨平台渠道对比分析师**（`marketplace-channel-comparison-analyst`，v1.0.5）
...
- **价格与促销分析师**（`pricing-promotion-analyst`，v1.0.5）
...
- **商品组合分析师**（`product-portfolio-analyst`，v1.0.5）
...
- **销售与利润分析师**（`sales-profitability-analyst`，v1.0.5）
...
- **流量与获客分析师**（`traffic-acquisition-analyst`，v1.0.5）
...
- **业务指标诊断师**（`metric-diagnostics`，v1.0.4）
  - 路径：`数据看板/metric-diagnostics/`
  - 分类：数据看板 / 业务分析 / 指标诊断
  - 用途：验证指标口径与数据质量，量化拆解异常变化并区分驱动因素和假设
- **业务数据质量与对账专家**（`data-quality-reconciliation`，v1.0.5）
...
- **争议解决案件经理**（`dispute-resolution-case-manager`，v1.0.5）
...
- **公司治理法务顾问**（`corporate-governance-counsel`，v1.0.5）
...
- **劳动用工合规顾问**（`employment-labor-compliance-counsel`，v1.0.5）
...
- **合同风险审阅师**（`contract-risk-reviewer`，v1.0.4）
  - 路径：`法律政务/contract-risk-reviewer/`
  - 分类：法律政务 / 合同 / 商业合同审阅
  - 用途：逐条提取合同义务、识别风险和缺失保护，并形成可谈判的优先清单
- **广告声明法律审查师**（`advertising-claims-legal-reviewer`，v1.0.5）
...
- **欧盟市场合规顾问**（`eu-market-regulatory-compliance-counsel`，v1.0.5）
...
- **跨境法务资深经理**（`senior-cross-border-legal-counsel`，v1.0.6）
...
- **跨境法务资深经理周报**（`senior-cross-border-legal-counsel-weekly-report`，v1.0.6）
...
- **知识产权保护顾问**（`intellectual-property-protection-counsel`，v1.0.5）
...
- **美国市场合规顾问**（`us-market-regulatory-compliance-counsel`，v1.0.5）
...
- **跨境监管合规顾问**（`cross-border-regulatory-compliance-counsel`，v1.0.5）
...
- **进出口贸易合规审查顾问**（`import-export-trade-compliance-reviewer`，v1.0.5）
...
- **隐私与数据保护顾问**（`privacy-data-protection-counsel`，v1.0.5）
...
- **跨境商品合规风险管理器**（`product-compliance-risk-manager`，v1.0.4）
...
- **定价与促销优化经理**（`pricing-promotion-optimizer`，v1.0.5）
...
- **平台商品信息优化师**（`marketplace-listing-optimizer`，v1.0.5）
...
- **亚马逊 PDP 导入草稿生成器**（`amazon-pdp-import-generator`，v2.1.2）
  - 路径：`渠道运营/多渠道运营/亚马逊/amazon-pdp-import-generator/`
  - 分类：渠道运营 / 多渠道运营 / 亚马逊
  - 用途：从商品图片和真实资料生成 SKU、变体与图片草稿，结合当前市场 Rank、趋势和 SEO/GEO 关键词证据生成 Amazon PDP，并完成上线前自检
- **Shopify 转化审计师**（`shopify-conversion-auditor`，v1.0.4）
  - 路径：`渠道运营/多渠道运营/独立站/shopify-conversion-auditor/`
  - 分类：渠道运营 / 多渠道运营 / 独立站
  - 用途：基于页面、数据和真实商品事实审计 Shopify 转化障碍并排序改进项
- **独立站产品上下架管理器**（`storefront-product-publishing-manager`，v1.0.4）
  - 路径：`渠道运营/多渠道运营/独立站/storefront-product-publishing-manager/`
  - 分类：渠道运营 / 多渠道运营 / 独立站
  - 用途：安全规划、执行和验证独立站产品上架、定时发布、下架、退市、归档、重定向与回滚
- **评价分析与品牌声誉经理**（`review-reputation-manager`，v1.0.6）
  - 路径：`渠道运营/客户体验/review-reputation-manager/`
  - 分类：渠道运营 / 客户体验
  - 用途：基于授权评价原文、星级、SKU、站点、时间及订单、退货、客服和 VOC 数据，合规分析主题、趋势、异常与根因，并推动产品、页面、履约和服务改进。适用于跨平台评价批量分析、差评、评分波动和异常评价诊断、评价主题与退货、客服、商品和履约联合分析、公开回复、升级和改进效果复盘，交付评价数据质量与健康报告、主题、情绪、严重度、趋势和异常分析、合规回复、根因、owner 和改进闭环、整改前后效果与剩余风险报告。
- **跨境渠道组合策略师**（`channel-portfolio-strategist`，v1.0.5）
...
- **TikTok Shop 运营经理**（`tiktok-shop-operations-manager`，v1.0.5）
...
- **亚马逊店铺运营经理**（`amazon-store-operations-manager`，v1.0.5）
...
- **Shopify 独立站运营经理**（`shopify-store-operations-manager`，v1.0.5）
...
- **电商单位经济模型分析师**（`ecommerce-unit-economics-analyst`，v1.0.4）
...
- **TikTok Shop 运营资深经理**（`senior-tiktok-shop-operations-manager`，v1.0.6）
...
- **TikTok Shop 运营资深经理周报**（`senior-tiktok-shop-operations-manager-weekly-report`，v1.0.6）
...
- **业务数据分析资深经理**（`senior-business-analytics-manager`，v1.0.6）
...
- **业务数据分析资深经理周报**（`senior-business-analytics-manager-weekly-report`，v1.0.6）
...
- **亚马逊运营资深经理**（`senior-amazon-operations-manager`，v1.0.6）
...
- **亚马逊运营资深经理周报**（`senior-amazon-operations-manager-weekly-report`，v1.0.6）
...
- **客户体验资深经理**（`senior-customer-experience-manager`，v1.0.6）
...
- **客户体验资深经理周报**（`senior-customer-experience-manager-weekly-report`，v1.0.6）
...
- **独立站运营资深经理**（`senior-dtc-operations-manager`，v1.0.6）
...
- **独立站运营资深经理周报**（`senior-dtc-operations-manager-weekly-report`，v1.0.6）
...
- **跨境电商资深经理**（`senior-cross-border-commerce-leader`，v1.0.6）
...
- **跨境电商资深经理周报**（`senior-cross-border-commerce-leader-weekly-report`，v1.0.6）
...
- **GEO 机器可读内容发布专家**（`geo-machine-readable-publisher`，v1.0.2）
  - 路径：`精准营销/GEO技术发布/geo-machine-readable-publisher/`
  - 分类：精准营销 / GEO技术发布
  - 用途：从合规静态资料与 PDP/PIM/CMS 等授权接口构建可追溯 GEO 可信知识库，生成和维护 llms.txt、llms-full.txt 与合规 JSON-LD Schema，并支持批量发布前后自检。适用于静态资料或 PDP/PIM/CMS 接口接入与可信知识库建设、单站 llms.txt 与 llms-full.txt 生成、商品、文章、组织和 FAQ Schema 批量生成、GEO 文件发布、抽样验证和版本更新，交付数据源、API、字段权限与可信度清单、带来源、版本、有效期和冲突状态的 GEO 可信知识库、llms.txt、llms-full.txt 和 JSON-LD 生成包、批量清单、自检报告、发布和回滚记录。
- **SEO 软文与社区内容分发专家**（`seo-editorial-community-distribution-specialist`，v1.0.1）
  - 路径：`精准营销/SEO内容分发/seo-editorial-community-distribution-specialist/`
  - 分类：精准营销 / SEO内容分发
  - 用途：从关键词与产品证据出发生成有独立价值的软文、客座稿、品牌帖和社区回复，并管理发布、互动、链接与效果复盘。适用于SEO 软文和客座文章生成、论坛、问答和社区品牌发帖、多站点内容分发、更新和效果复盘，交付关键词与选题证据包、渠道适配软文、帖子和回复草稿、发布矩阵、自检、互动和效果复盘。
- **跨境内容营销规划师**（`content-marketing-planner`，v1.0.5）
...
- **跨境品牌定位策略师**（`brand-positioning-strategist`，v1.0.5）
...
- **跨境全链路增长负责人**（`cross-border-growth-operator`，v1.0.5）
...
- **跨渠道营销归因规划师**（`marketing-attribution-planner`，v1.0.5）
...
- **跨境市场情报分析师**（`cross-border-market-intelligence`，v1.0.5）
...
- **跨境上市规划师**（`go-to-market-launch-planner`，v1.0.5）
...
- **独立站 Google Ads 投放专家**（`google-ads-ecommerce-specialist`，v1.0.1）
  - 路径：`精准营销/投放广告/Google/google-ads-ecommerce-specialist/`
  - 分类：精准营销 / 投放广告 / Google
  - 用途：围绕搜索意图、商品 Feed、落地页、转化价值和贡献利润规划与优化 Google Ads。适用于Google Search、Shopping 与 Performance Max 投放、Merchant Center、Feed 和转化跟踪诊断、搜索词治理、预算扩量和利润止损，交付Google Ads 投放与测量方案、关键词、搜索词、Feed、资产组和预算矩阵、诊断、实验、自检和复盘记录。
- **独立站 Meta 广告投放专家**（`meta-ads-ecommerce-specialist`，v1.0.1）
  - 路径：`精准营销/投放广告/Meta/meta-ads-ecommerce-specialist/`
  - 分类：精准营销 / 投放广告 / Meta
  - 用途：以独立站真实利润、新客质量和可验证增量为目标，端到端规划、搭建、诊断和扩量 Meta 广告。适用于Meta 独立站冷启动和新品投放、Pixel 与 Conversions API 测量治理、账户诊断、素材测试、扩量和止损，交付Meta 投放与测量方案、Campaign、广告组、素材和预算矩阵、诊断、实验、自检和复盘记录。
- **多渠道电商广告优化师**（`commerce-advertising-optimizer`，v1.0.4）
...
- **用户生命周期、会员与挽留经理**（`customer-retention-loyalty-manager`，v1.0.6）
  - 路径：`精准营销/用户运营/customer-retention-loyalty-manager/`
  - 分类：精准营销 / 用户运营
  - 用途：基于统一客户身份、同意、客户价值和行为周期，设计新客培育、老客复购、会员权益、流失预警、召回挽留和忠诚增长闭环。适用于新客首购后培育与二购提升、新老客分层、生命周期旅程和老客经营、会员、等级、积分、权益和推荐计划、沉默、流失预警、召回与挽留、留存下降、会员成本或复购质量诊断，交付新客、老客、会员和流失定义及数据质量报告、生命周期分层、状态机、触发、频控和跨渠道旅程、会员权益、积分负债、成本、毛利和体验模型、召回挽留实验、增量评估、止损与复盘。
- **邮件生命周期营销师**（`email-lifecycle-marketer`，v1.0.5）
...
- **跨境社媒运营经理**（`social-media-operations-manager`，v1.0.5）
...
- **品牌与内容资深经理**（`senior-brand-content-manager`，v1.0.6）
...
- **品牌与内容资深经理周报**（`senior-brand-content-manager-weekly-report`，v1.0.6）
...
- **增长营销资深经理**（`senior-growth-marketing-manager`，v1.0.6）
...
- **增长营销资深经理周报**（`senior-growth-marketing-manager-weekly-report`，v1.0.6）
...
- **用户运营资深经理**（`senior-user-operations-manager`，v1.0.6）
...
- **用户运营资深经理周报**（`senior-user-operations-manager-weekly-report`，v1.0.6）
...
- **达人与联盟营销经理**（`influencer-affiliate-manager`，v1.0.5）
...
- **资金出纳资深经理**（`senior-cashier-treasury-specialist`，v1.0.6）
...
- **资金出纳资深经理周报**（`senior-cashier-treasury-specialist-weekly-report`，v1.0.7）
...
- **报销与票据出纳专员**（`expense-reimbursement-cashier`，v1.0.5）
...
- **收付款执行管理员**（`payment-receipt-operations-manager`，v1.0.5）
...
- **财务关账与对账经理**（`financial-close-reconciliation-manager`，v1.0.5）
...
- **跨境税务与 VAT 规划师**（`cross-border-tax-vat-planner`，v1.0.5）
...
- **经营财务与利润经理**（`management-accounting-profitability-manager`，v1.0.5）
...
- **跨境财务资深经理**（`senior-cross-border-finance-manager`，v1.0.6）
...
- **跨境财务资深经理周报**（`senior-cross-border-finance-manager-weekly-report`，v1.0.6）
...
- **资金安全与支付权限控制专员**（`cash-security-access-controller`，v1.0.5）
...
- **现金流与营运资金经理**（`cash-flow-working-capital-manager`，v1.0.5）
...
- **多币种资金与汇率经理**（`multi-currency-treasury-manager`，v1.0.5）
...
- **银行账户与出纳对账专员**（`bank-account-reconciliation-cashier`，v1.0.5）
...
- **预算与预测控制经理**（`budget-forecast-controller`，v1.0.5）
