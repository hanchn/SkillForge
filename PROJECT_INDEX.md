...
- **CMS 产品架构师**（`cms-product-architecture`，v1.0.2）
...
- **CRM 产品架构师**（`crm-product-architecture`，v1.0.2）
...
- **IMS 产品架构师**（`ims-product-architecture`，v1.0.2）
...
- **OFS 产品架构师**（`ofs-product-architecture`，v1.0.2）
...
- **OMS 产品架构师**（`oms-product-architecture`，v1.0.2）
...
- **PLM 产品架构师**（`plm-product-architecture`，v1.0.2）
...
- **TMS 产品架构师**（`tms-product-architecture`，v1.0.2）
...
- **WMS 产品架构师**（`wms-product-architecture`，v1.0.2）
...
- **跨系统产品架构师**（`commerce-systems-product-architecture`，v1.1.2）
...
- **UI 交互设计与实现专家**（`ui-interaction-designer`，v1.0.2）
...
- **前端功能实现工程师**（`frontend-feature-implementer`，v1.0.2）
...
- **前端多语言查验师**（`frontend-localization-verifier`，v1.0.2）
...
- **前端性能分析师**（`frontend-performance-analyzer`，v1.0.2）
...
- **Web 性能与无障碍审计师**（`web-performance-accessibility-auditor`，v1.0.2）
...
- **前端架构师**（`frontend-architecture-designer`，v1.0.3）
...
- **React 资深专家**（`react-senior-expert`，v1.0.3）
...
- **Vue3 资深专家**（`vue3-senior-expert`，v1.0.3）
...
- **API 契约设计师**（`api-contract-designer`，v1.0.2）
  - 路径：`互联网研发/后端/api-contract-designer/`
  - 分类：互联网研发 / 后端 / API设计
  - 用途：设计一致、可演进、可测试并能直接评审的 HTTP 或事件 API 契约
- **数据库变更迁移规划师**（`database-migration-planner`，v1.0.2）
...
- **后端服务架构师**（`backend-service-architect`，v1.0.2）
...
- **后端架构师**（`backend-architecture-designer`，v1.0.3）
...
- **ASP.NET Core 后端资深专家**（`aspnet-core-backend-architecture`，v1.0.3）
...
- **FastAPI 后端资深专家**（`fastapi-backend-architecture`，v1.0.3）
...
- **Gin 后端资深专家**（`gin-backend-architecture`，v1.0.3）
...
- **Laravel 后端资深专家**（`laravel-backend-architecture`，v1.0.3）
...
- **NestJS 后端资深专家**（`nestjs-backend-architecture`，v1.0.3）
...
- **Spring Boot 后端资深专家**（`spring-boot-backend-architecture`，v1.0.3）
...
- **生产故障诊断师**（`production-incident-diagnostician`，v1.0.2）
...
- **C# 后端资深专家**（`csharp-backend-architecture`，v1.0.3）
...
- **Go 后端资深专家**（`go-backend-architecture`，v1.0.3）
...
- **Java 后端资深专家**（`java-backend-architecture`，v1.0.3）
...
- **Node.js 后端资深专家**（`nodejs-backend-architecture`，v1.0.3）
...
- **PHP 后端资深专家**（`php-backend-architecture`，v1.0.3）
...
- **Python 后端资深专家**（`python-backend-architecture`，v1.0.3）
...
- **Rust 后端资深专家**（`rust-backend-architecture`，v1.0.3）
...
- **测试策略设计师**（`test-strategy-designer`，v1.0.2）
  - 路径：`互联网研发/测试/test-strategy-designer/`
  - 分类：互联网研发 / 测试 / 测试策略
  - 用途：根据变更影响和风险设计分层覆盖、发布门槛与生产验证方案
- **全栈开发资深专家**（`fullstack-feature-delivery`，v1.0.4）
...
- **全栈开发资深专家周报**（`fullstack-developer-weekly-report`，v1.0.3）
...
- **全栈测试资深专家**（`fullstack-quality-engineer`，v1.0.4）
...
- **全栈测试资深专家周报**（`fullstack-qa-weekly-report`，v1.0.3）
...
- **人事招聘资深经理**（`senior-hr-recruiting-manager`，v1.0.3）
...
- **人事招聘资深经理周报**（`senior-hr-recruiting-manager-weekly-report`，v1.0.4）
...
- **候选人寻访运营专员**（`candidate-sourcing-operator`，v1.0.2）
...
- **人事招聘通用执行专家**（`hr-recruitment-generalist`，v1.0.2）
...
- **绩效与员工关系管理专员**（`performance-employee-relations-manager`，v1.0.2）
...
- **企业员工公告撰写专家**（`employee-announcement-writer`，v1.0.2）
...
- **员工生命周期与档案管理员**（`employee-lifecycle-records-manager`，v1.0.2）
...
- **员工培训组织与运营经理**（`employee-training-organization-manager`，v1.0.2）
...
- **岗位说明书与招聘 JD 生成专家**（`job-description-generator`，v1.0.3）
...
- **录用与入职管理专员**（`offer-onboarding-manager`，v1.0.2）
...
- **人力编制与招聘规划师**（`workforce-recruiting-planner`，v1.0.2）
...
- **结构化面试与评估设计师**（`structured-interview-designer`，v1.0.2）
...
- **仓储库存资深经理**（`senior-warehouse-inventory-manager`，v1.0.3）
...
- **仓储库存资深经理周报**（`senior-warehouse-inventory-manager-weekly-report`，v1.0.3）
...
- **供应链资深经理**（`senior-supply-chain-manager`，v1.0.3）
...
- **供应链资深经理周报**（`senior-supply-chain-manager-weekly-report`，v1.0.3）
...
- **仓容、现场安全与连续性经理**（`warehouse-safety-capacity-manager`，v1.0.2）
...
- **仓库入库作业经理**（`warehouse-inbound-operations-manager`，v1.0.2）
...
- **仓库出库履约经理**（`warehouse-outbound-fulfillment-manager`，v1.0.2）
...
- **电商履约物流规划师**（`ecommerce-fulfillment-logistics-planner`，v1.0.2）
...
- **多仓多渠道库存分配规划师**（`inventory-allocation-planner`，v1.0.2）
...
- **库存控制与循环盘点经理**（`inventory-control-cycle-count-manager`，v1.0.2）
...
- **多渠道库存补货规划师**（`inventory-replenishment-planner`，v2.0.2）
...
- **退货与逆向物流经理**（`returns-reverse-logistics-manager`，v1.0.2）
...
- **AIGC 创意制作专家**（`ai-image-video-production-manager`，v1.0.3）
...
- **智能剪辑工作流专家**（`intelligent-video-editing-specialist`，v1.0.3）
...
- **创意素材智能打标专家**（`creative-asset-intelligent-tagging-specialist`，v1.0.3）
...
- **AI 创意真实性与合规审查师**（`ai-creative-authenticity-compliance-reviewer`，v1.0.3）
...
- **虚拟模特内容制作专家**（`virtual-model-content-producer`，v1.0.3）
...
- **UGC 与达人内容制作经理**（`ugc-content-production-manager`，v1.0.3）
...
- **跨境广告创意策略师**（`creative-strategy-producer`，v1.0.3）
...
- **创拍视觉资深经理**（`senior-creative-production-manager`，v1.0.4）
...
- **创拍视觉资深经理周报**（`senior-creative-production-manager-weekly-report`，v1.0.4）
...
- **剪辑后期与素材交付经理**（`post-production-delivery-manager`，v1.0.3）
...
- **商品摄影导演**（`product-photography-director`，v1.0.3）
...
- **商品图片与视频拍摄制作经理**（`product-photo-video-production-manager`，v1.0.3）
...
- **拍摄制片与现场统筹**（`shooting-production-coordinator`，v1.0.3）
...
- **跨境短视频制作导演**（`short-form-video-producer`，v1.0.3）
...
- **创意素材运营经理**（`creative-asset-operations-manager`，v1.0.3）
...
- **客服售前资深经理**（`senior-customer-service-presales-manager`，v1.0.3）
...
- **客服售前资深经理周报**（`senior-customer-service-presales-manager-weekly-report`，v1.0.3）
...
- **跨境客服售前运营专家**（`customer-service-voice-manager`，v2.0.2）
...
- **供应商寻源与质量管理器**（`supplier-sourcing-quality-manager`，v1.0.2）
...
- **供应商谈判与成本经理**（`supplier-negotiation-cost-manager`，v1.0.2）
...
- **市场需求与商品机会分析师**（`market-demand-opportunity-analyst`，v1.0.2）
...
- **采购样品验证经理**（`sample-evaluation-sourcing-manager`，v1.0.2）
...
- **跨境电商选品研究**（`ecommerce-product-research`，v1.0.2）
  - 路径：`市场采购/选品/ecommerce-product-research/`
  - 分类：市场采购 / 选品
  - 用途：从需求、竞争、利润、供应链、合规和渠道适配评估跨境产品机会
- **采购订单与交付经理**（`purchase-order-delivery-manager`，v1.0.2）
...
- **市场采购资深经理**（`senior-market-procurement-manager`，v1.0.3）
...
- **市场采购资深经理周报**（`senior-market-procurement-manager-weekly-report`，v1.0.3）
...
- **选品与商品资深经理**（`senior-merchandising-manager`，v1.0.3）
...
- **选品与商品资深经理周报**（`senior-merchandising-manager-weekly-report`，v1.0.3）
...
- **采购风险与供应韧性经理**（`procurement-risk-compliance-manager`，v1.0.2）
...
- **广告效果分析师**（`advertising-performance-analyst`，v1.0.2）
...
- **业务指标治理师**（`business-metrics-governance`，v1.0.2）
...
- **电商转化漏斗分析师**（`conversion-funnel-analyst`，v1.0.2）
...
- **客户 Cohort 与 LTV 分析师**（`customer-cohort-ltv-analyst`，v1.0.2）
...
- **经营分析与周月报专家**（`executive-business-review`，v1.0.2）
...
- **经营预测与情景规划师**（`forecast-scenario-planner`，v1.0.2）
...
- **库存与供应链分析师**（`inventory-supply-chain-analyst`，v1.0.2）
...
- **跨平台渠道对比分析师**（`marketplace-channel-comparison-analyst`，v1.0.2）
...
- **价格与促销分析师**（`pricing-promotion-analyst`，v1.0.2）
...
- **商品组合分析师**（`product-portfolio-analyst`，v1.0.2）
...
- **销售与利润分析师**（`sales-profitability-analyst`，v1.0.2）
...
- **流量与获客分析师**（`traffic-acquisition-analyst`，v1.0.2）
...
- **业务指标诊断师**（`metric-diagnostics`，v1.0.2）
  - 路径：`数据看板/metric-diagnostics/`
  - 分类：数据看板 / 业务分析 / 指标诊断
  - 用途：验证指标口径与数据质量，量化拆解异常变化并区分驱动因素和假设
- **业务数据质量与对账专家**（`data-quality-reconciliation`，v1.0.2）
...
- **争议解决案件经理**（`dispute-resolution-case-manager`，v1.0.2）
...
- **公司治理法务顾问**（`corporate-governance-counsel`，v1.0.2）
...
- **劳动用工合规顾问**（`employment-labor-compliance-counsel`，v1.0.2）
...
- **合同风险审阅师**（`contract-risk-reviewer`，v1.0.2）
  - 路径：`法律政务/contract-risk-reviewer/`
  - 分类：法律政务 / 合同 / 商业合同审阅
  - 用途：逐条提取合同义务、识别风险和缺失保护，并形成可谈判的优先清单
- **广告声明法律审查师**（`advertising-claims-legal-reviewer`，v1.0.2）
...
- **欧盟市场合规顾问**（`eu-market-regulatory-compliance-counsel`，v1.0.2）
...
- **跨境法务资深经理**（`senior-cross-border-legal-counsel`，v1.0.3）
...
- **跨境法务资深经理周报**（`senior-cross-border-legal-counsel-weekly-report`，v1.0.3）
...
- **知识产权保护顾问**（`intellectual-property-protection-counsel`，v1.0.2）
...
- **美国市场合规顾问**（`us-market-regulatory-compliance-counsel`，v1.0.2）
...
- **跨境监管合规顾问**（`cross-border-regulatory-compliance-counsel`，v1.0.2）
...
- **进出口贸易合规审查顾问**（`import-export-trade-compliance-reviewer`，v1.0.2）
...
- **隐私与数据保护顾问**（`privacy-data-protection-counsel`，v1.0.2）
...
- **跨境商品合规风险管理器**（`product-compliance-risk-manager`，v1.0.2）
...
- **定价与促销优化经理**（`pricing-promotion-optimizer`，v1.0.2）
...
- **平台商品信息优化师**（`marketplace-listing-optimizer`，v1.0.2）
...
- **亚马逊 PDP 导入草稿生成器**（`amazon-pdp-import-generator`，v2.0.3）
...
- **跨境渠道组合策略师**（`channel-portfolio-strategist`，v1.0.2）
...
- **TikTok Shop 运营经理**（`tiktok-shop-operations-manager`，v1.0.2）
...
- **亚马逊店铺运营经理**（`amazon-store-operations-manager`，v1.0.2）
...
- **Shopify 独立站运营经理**（`shopify-store-operations-manager`，v1.0.2）
...
- **电商单位经济模型分析师**（`ecommerce-unit-economics-analyst`，v1.0.2）
...
- **TikTok Shop 运营资深经理**（`senior-tiktok-shop-operations-manager`，v1.0.3）
...
- **TikTok Shop 运营资深经理周报**（`senior-tiktok-shop-operations-manager-weekly-report`，v1.0.3）
...
- **业务数据分析资深经理**（`senior-business-analytics-manager`，v1.0.3）
...
- **业务数据分析资深经理周报**（`senior-business-analytics-manager-weekly-report`，v1.0.3）
...
- **亚马逊运营资深经理**（`senior-amazon-operations-manager`，v1.0.3）
...
- **亚马逊运营资深经理周报**（`senior-amazon-operations-manager-weekly-report`，v1.0.3）
...
- **客户体验资深经理**（`senior-customer-experience-manager`，v1.0.3）
...
- **客户体验资深经理周报**（`senior-customer-experience-manager-weekly-report`，v1.0.3）
...
- **独立站运营资深经理**（`senior-dtc-operations-manager`，v1.0.3）
...
- **独立站运营资深经理周报**（`senior-dtc-operations-manager-weekly-report`，v1.0.3）
...
- **跨境电商资深经理**（`senior-cross-border-commerce-leader`，v1.0.3）
...
- **跨境电商资深经理周报**（`senior-cross-border-commerce-leader-weekly-report`，v1.0.3）
...
- **跨境内容营销规划师**（`content-marketing-planner`，v1.0.2）
...
- **跨境品牌定位策略师**（`brand-positioning-strategist`，v1.0.2）
...
- **跨境全链路增长负责人**（`cross-border-growth-operator`，v1.0.2）
...
- **跨渠道营销归因规划师**（`marketing-attribution-planner`，v1.0.2）
...
- **新客获取与拉新增长经理**（`customer-acquisition-growth-manager`，v1.0.2）
...
- **跨境市场情报分析师**（`cross-border-market-intelligence`，v1.0.2）
...
- **跨境上市规划师**（`go-to-market-launch-planner`，v1.0.2）
...
- **多渠道电商广告优化师**（`commerce-advertising-optimizer`，v1.0.2）
...
- **客户留存与忠诚度经理**（`customer-retention-loyalty-manager`，v1.0.2）
...
- **邮件生命周期营销师**（`email-lifecycle-marketer`，v1.0.2）
...
- **跨境社媒运营经理**（`social-media-operations-manager`，v1.0.2）
...
- **品牌与内容资深经理**（`senior-brand-content-manager`，v1.0.3）
...
- **品牌与内容资深经理周报**（`senior-brand-content-manager-weekly-report`，v1.0.3）
...
- **增长营销资深经理**（`senior-growth-marketing-manager`，v1.0.3）
...
- **增长营销资深经理周报**（`senior-growth-marketing-manager-weekly-report`，v1.0.3）
...
- **用户运营资深经理**（`senior-user-operations-manager`，v1.0.3）
...
- **用户运营资深经理周报**（`senior-user-operations-manager-weekly-report`，v1.0.3）
...
- **达人与联盟营销经理**（`influencer-affiliate-manager`，v1.0.2）
...
- **资金出纳资深经理**（`senior-cashier-treasury-specialist`，v1.0.3）
...
- **资金出纳资深经理周报**（`senior-cashier-treasury-specialist-weekly-report`，v1.0.4）
...
- **报销与票据出纳专员**（`expense-reimbursement-cashier`，v1.0.2）
...
- **收付款执行管理员**（`payment-receipt-operations-manager`，v1.0.2）
...
- **财务关账与对账经理**（`financial-close-reconciliation-manager`，v1.0.2）
...
- **跨境税务与 VAT 规划师**（`cross-border-tax-vat-planner`，v1.0.2）
...
- **经营财务与利润经理**（`management-accounting-profitability-manager`，v1.0.2）
...
- **跨境财务资深经理**（`senior-cross-border-finance-manager`，v1.0.3）
...
- **跨境财务资深经理周报**（`senior-cross-border-finance-manager-weekly-report`，v1.0.3）
...
- **资金安全与支付权限控制专员**（`cash-security-access-controller`，v1.0.2）
...
- **现金流与营运资金经理**（`cash-flow-working-capital-manager`，v1.0.2）
...
- **多币种资金与汇率经理**（`multi-currency-treasury-manager`，v1.0.2）
...
- **银行账户与出纳对账专员**（`bank-account-reconciliation-cashier`，v1.0.2）
...
- **预算与预测控制经理**（`budget-forecast-controller`，v1.0.2）
