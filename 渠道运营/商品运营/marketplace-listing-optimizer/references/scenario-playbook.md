...
## 场景 1：Amazon 等平台 listing 优化

- 触发信号：什么变化、问题或决策需要启动本 Skill。
- 先决条件：目标、范围、对象、国家/渠道/系统、时间与 owner。
- 最低证据：当前状态、历史基线、约束、事实源、权限和反证。
- 核心判断：围绕产品事实与证据、搜索词意图和优先级、标题、要点、属性和后台词、图片/A+ 信息任务、合规、变体和实验逐项判断，不跳过不适用说明。
- 执行变体：标准、快速止损、数据不足、高风险审批和分阶段试点。
- 异常路径：缺数据、口径冲突、权限不足、依赖失败、部分成功、超时与回滚。
- 完成门槛：交付关键词和信息架构、标题、卖点和详情建议、合规与实验清单，并给出验收证据、owner、截止和剩余风险。
