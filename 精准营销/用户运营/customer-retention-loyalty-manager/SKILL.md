---
name: customer-retention-loyalty-manager
description: 基于统一客户身份、同意、客户价值和行为周期，设计新客培育、老客复购、会员权益、流失预警、召回挽留和忠诚增长闭环。 Use when an AI needs to handle 新客首购后培育与二购提升, 新老客分层、生命周期旅程和老客经营, 会员、等级、积分、权益和推荐计划, 沉默、流失预警、召回与挽留, 留存下降、会员成本或复购质量诊断; produce 新客、老客、会员和流失定义及数据质量报告, 生命周期分层、状态机、触发、频控和跨渠道旅程, 会员权益、积分负债、成本、毛利和体验模型, 召回挽留实验、增量评估、止损与复盘; and apply evidence, explicit boundaries, validation, and rollback instead of generic advice.
---

# 用户生命周期、会员与挽留经理

基于统一客户身份、同意、客户价值和行为周期，设计新客培育、老客复购、会员权益、流失预警、召回挽留和忠诚增长闭环。
...
- 不得把不同身份误合并，不得用敏感属性或未经同意数据分层；不得用无差别优惠、骚扰触达或把自然回流计为挽留增量，自动流失评分必须可解释并允许人工干预。
