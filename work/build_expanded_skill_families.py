...
SPECS = [
    ...
    spec("精准营销/创意生产/creative-strategy-producer", "creative-strategy-producer", "跨境广告创意策略师", "operations", "从人群洞察和产品证据建立可批量测试、可归因的创意生产系统。", ["广告素材策略", "UGC brief 和脚本矩阵", "创意疲劳治理"], ["创意策略地图", "角度、钩子和脚本矩阵", "测试编码与复盘规则"], ["人群问题和欲望", "产品证据与演示", "角度、钩子、格式和 CTA", "平台原生表达", "变量隔离、标签和疲劳"], "不得虚构效果、评价或前后对比。"),
    ...
    spec("渠道运营/客户体验/customer-service-voice-manager", "customer-service-voice-manager", "跨境多语言客服经理", "operations", "以多语言一致性、问题解决和客户洞察运营售前售后服务。", ["邮件、在线聊天和平台消息客服", "多语言话术体系", "客服质检和 VOC 闭环"], ["客服 SOP 与话术库", "升级和赔付矩阵", "VOC 分类与改进报告"], ["语言、语气和文化", "意图识别和信息核验", "政策、权限和升级", "SLA、一次解决率和质检", "VOC 到产品运营闭环"], "不得承诺超出政策或权限的结果，也不得机械直译敏感回复。"),
    ...
    spec("互联网研发/产品/系统产品架构/公共/commerce-systems-product-architecture", "commerce-systems-product-architecture", "跨境业务系统总产品架构师", "product-system", "设计 OMS、IMS、OFS、CMS、WMS、TMS、CRM、PLM 等系统的职责地图、主数据归属和端到端协同。", ["跨境业务系统蓝图", "系统边界重构", "新系统立项和集成治理"], ["业务能力与系统地图", "主数据和状态 ownership 矩阵", "跨系统流程、契约与演进路线"], ["业务能力和端到端旅程", "系统职责与不负责范围", "主数据和 source of truth", "命令、事件、状态和异常", "集成、SLA、审计和分阶段建设"], "不得先按现有部门或系统名称切边界，必须先锁定业务能力与贵司缩写口径。"),
...
ROLE_DEFS = [
    ...
    ("渠道运营/运营角色", "选品与商品高级经理", "senior-merchandising-manager", "选品与商品高级经理", "统筹市场机会、选品验证、商品组合、生命周期、定价、内容准备和退出机制。", ["市场机会和需求证据", "选品门槛与单位经济", "SKU 组合和生命周期", "定价、内容和上市准备", "库存风险和退出机制"]),
    ...
    ("渠道运营/运营角色", "供应链高级经理", "senior-supply-chain-manager", "供应链高级经理", "统筹供应商、质量、采购、库存、仓储、运输、履约和供应链现金风险。", ["供应商和质量体系", "需求、采购和补货", "库存、仓储和渠道分配", "物流、履约和异常", "成本、现金、SLA 和韧性"]),
...
for role_area, folder, role_id, display, positioning, lenses in ROLE_DEFS:
    SPECS.append(spec(
        f"{role_area}/{folder}/{role_id}", role_id, display, "role",
        positioning,
...
CATEGORIES = {
    ...
    "精准营销/创意生产": ("创意生产", "覆盖洞察、角度、脚本、素材测试和疲劳管理。"),
    "渠道运营/商品运营": ("商品运营", "覆盖平台商品信息、搜索发现和转化表达。"),
    "渠道运营/客户体验": ("客户体验", "覆盖多语言客服、评价、声誉和 VOC 改进闭环。"),
    ...
    "互联网研发/产品/系统产品架构/公共": ("业务系统公共架构", "沉淀跨系统能力地图、主数据归属和端到端协同。"),
...
CATEGORIES.update({
    ...
    "财务出纳": ("财务出纳", "覆盖跨境财务主角色、关账对账、预算预测、现金、税务、外汇和经营财务。参见 [财务 Skill 地图](FINANCE_SKILLS_MAP.md)。"),
    "财务出纳/财务角色": ("财务主角色", "沉淀跨境财务高级经理及专属周报 Skill。"),
    "财务出纳/财务角色/跨境财务高级经理": ("跨境财务高级经理", "统筹会计、管理报表、预算、现金、税务、外汇和内控。"),
...
def category_readme(relative, title, summary):
    ...
    if top == "法律政务":
        ...
        principles = ["先确认主体、期间、币种、账簿和法定/管理口径。", "跨境财务高级经理负责总体结果，专项 Skill 负责关账、预算、资金、税务或经营财务。", "预估、未关账和管理数据不得冒充正式财务结果。"]
