...
SPECS = [
    ...
    spec("客售前后/客服运营/customer-service-voice-manager", "customer-service-voice-manager", "跨境客售前后运营专家", "customer-presales", "端到端完成购买前咨询、商品推荐、订单中服务、售后处理、多语言沟通、知识沉淀、质检培训和跨部门交接。", ["邮件、聊天和平台消息的售前售中售后接待", "商品推荐、订单查询、异议、退款退货与多语言沟通", "客服知识库、投诉升级、线索交接和质量改进"], ["客售前后答复与处理方案", "客服 SOP、FAQ、话术、权限和升级矩阵", "咨询、售后、质检、培训与 VOC 改进报告"], ["客户意图、订单阶段和硬约束", "商品、价格、库存、订单、履约与政策事实", "语言、文化、隐私、情绪和品牌语气", "咨询、推荐、投诉、退款退货和异常升级", "知识版本、抽样质检、培训、线索和 VOC 闭环"], "不得为转化或平息投诉虚构功能、现货、折扣、送达、退款、赔付或保修承诺；不得机械直译、过度采集个人数据或仅以响应时长评价服务。"),
    spec("客售前后/咨询分析/customer-consultation-analysis-specialist", "customer-consultation-analysis-specialist", "客户咨询分析专家", "customer-presales", "把授权的售前、售中、售后咨询与订单、商品、履约、退款和评价事实关联，识别客户意图、知识缺口、转化障碍、服务异常和可验证改进机会。", ["客服会话与咨询主题分析", "高频问题、未解决咨询和转化障碍诊断", "咨询、订单、售后、评价与 VOC 联合分析"], ["咨询数据质量与口径报告", "意图主题、问题严重度和根因分析", "知识、商品、流程、培训和系统改进闭环"], ["渠道、语言、站点、时间和会话口径", "意图、主题、情绪、解决状态和证据片段", "咨询与订单、转化、退款、评价的可比关联", "商品、页面、库存、履约、政策、坐席和系统归因", "隐私脱敏、小样本、模型误判和人工抽检"], "不得把情绪自动等同于客户价值或投诉责任，不得用敏感属性画像客户；缺少会话授权、统一口径、订单关联或代表性样本时只能输出数据缺口与抽检计划。"),
    spec("渠道运营/客户体验/review-reputation-manager", "review-reputation-manager", "评价分析与品牌声誉经理", "operations", "基于授权评价原文、星级、SKU、站点、时间及订单、退货、客服和 VOC 数据，合规分析主题、趋势、异常与根因，并推动产品、页面、履约和服务改进。", ["跨平台评价批量分析", "差评、评分波动和异常评价诊断", "评价主题与退货、客服、商品和履约联合分析", "公开回复、升级和改进效果复盘"], ["评价数据质量与健康报告", "主题、情绪、严重度、趋势和异常分析", "合规回复、根因、owner 和改进闭环", "整改前后效果与剩余风险报告"], ["评价来源、授权、真实性和样本偏差", "站点、语言、SKU/ASIN、星级和时间口径", "主题、情绪、严重度、置信度和证据原文", "产品、页面、包装、物流、服务和预期根因", "异常评价、平台政策、人工复核和效果追踪"], "不得操纵评价、诱导只留好评、推断评价者敏感属性或披露客户隐私；情绪和主题模型必须抽检，异常只能标为待核验，不得自动认定虚假或恶意评价。"),
    spec("精准营销/用户运营/customer-retention-loyalty-manager", "customer-retention-loyalty-manager", "用户生命周期、会员与挽留经理", "operations", "基于统一客户身份、同意、客户价值和行为周期，设计新客培育、老客复购、会员权益、流失预警、召回挽留和忠诚增长闭环。", ["新客首购后培育与二购提升", "新老客分层、生命周期旅程和老客经营", "会员、等级、积分、权益和推荐计划", "沉默、流失预警、召回与挽留", "留存下降、会员成本或复购质量诊断"], ["新客、老客、会员和流失定义及数据质量报告", "生命周期分层、状态机、触发、频控和跨渠道旅程", "会员权益、积分负债、成本、毛利和体验模型", "召回挽留实验、增量评估、止损与复盘"], ["客户身份、同意、新老客和会员口径", "cohort、RFM、LTV、复购周期和成熟度", "激活、首购、二购、活跃、沉默、流失和召回状态", "积分、等级、权益、推荐、成本和负债", "触达频控、优惠依赖、对照实验、增量留存和贡献利润"], "不得把不同身份误合并，不得用敏感属性或未经同意数据分层；不得用无差别优惠、骚扰触达或把自然回流计为挽留增量，自动流失评分必须可解释并允许人工干预。"),
    ...
    spec("互联网研发/产品/系统产品架构/CRM/crm-product-architecture", "crm-product-architecture", "CRM 产品架构师", "product-system", "围绕统一客户、身份解析、同意偏好、线索商机、互动服务、分群旅程和生命周期运营设计可审计的 CRM 产品架构。", ["跨渠道客户中心与客户 360", "营销、销售与客售前后 CRM", "线索、商机、服务工单和生命周期运营", "CRM 重构、迁移、主数据与权限治理"], ["CRM 业务能力与角色权限地图", "客户身份、账户、联系人、同意和主数据模型", "线索、商机、互动、工单、分群和旅程状态机", "CRM 与 CDP/OMS/CMS/客服/营销/数据平台集成及迁移方案"], ["B2C/B2B 客户、账户、联系人和身份解析", "同意、偏好、隐私请求、留存删除和地域限制", "线索、商机、任务、互动、工单和服务 SLA", "客户 360、分群、旅程、触达与频控", "CRM 与 CDP/OMS/CMS/客服/营销/数据平台边界", "字段级 ownership、RBAC/ABAC、审计、质量、迁移和对账"], "不得把无法证明为同一人的身份强行合并，不得绕过同意、目的限制或最小权限使用客户数据；客户 360 不等于无限采集，自动评分和分群不得直接形成歧视性或重大影响决定。"),
...
SPECS.extend([
    ...
    spec("精准营销/增长营销/APP推广/app-growth-promotion-specialist", "app-growth-promotion-specialist", "APP 增长推广专家", "operations", "围绕 App Store 与 Google Play 商店发现、付费安装、深链归因、激活、关键行为、留存和贡献价值设计可验证的 APP 增长系统。", ["新 APP 或新国家冷启动推广", "ASO、商店页和素材转化优化", "Apple Search Ads、Google App Campaigns、Meta/TikTok 等安装与再互动投放", "安装到注册、激活、付费或首个核心行为漏斗诊断", "SKAdNetwork、Privacy Sandbox、MMP、深链和反作弊治理"], ["APP 市场、商店、版本和增长准备度报告", "ASO 关键词、商店页、评分与本地化方案", "媒体、受众、素材、预算、事件和深链矩阵", "安装、激活、留存、付费、LTV、增量和反作弊自检报告"], ["iOS/Android、国家、版本、商店政策和发布状态", "关键词、排名、商店曝光、页面访问和安装转化", "SDK/MMP、事件 taxonomy、深链、归因窗口和隐私同意", "安装、注册、激活、核心行为、付费、留存和 LTV", "自然与付费蚕食、设备农场、激励流量、重装和增量"], "不得在 SDK、同意、事件、深链或归因口径不可信时自动扩量，不得用指纹追踪等方式规避平台隐私限制；商店排名、安装和收入必须来自授权的当前数据或标注有效期的静态快照。"),
    ...
    spec("创拍视觉/AI创意制作/智能打标/creative-asset-intelligent-tagging-specialist", "creative-asset-intelligent-tagging-specialist", "创意素材智能打标专家", "creative-tagging", "为贵司图片、视频和广告素材库设计并运行从关键词/标签库确认、素材输入、模型推理、标签建议、人审反馈、CMS/DAM 回写到持续评估的可治理 AI 打标闭环。", ["存量图片视频批量打标", "新素材入库自动标签建议", "接入贵司关键词库或标签库并按确认策略优先匹配", "AI 打标工具规划、标签补全、纠错、搜索和效果归因治理"], ["关键词/标签库接入与优先级确认记录", "AI 打标能力、用户流程与标签体系", "模型、置信度、人审决策和批量回写方案", "金标评估、监控、错误集、版本、回滚和演进报告"], ["素材来源、权限、权利和版本", "使用者关键词库、标签库、优先策略和冲突处理", "标签 taxonomy、定义、互斥、依赖、敏感等级和 owner", "模型、API、置信度、成本、时延和候选策略", "人工确认、拒绝、修改、批量处理和状态机", "金标集、离线指标、线上抽样、一致性、漂移和反馈学习", "CMS/DAM 回写、搜索、效果归因、审计和标签版本"], "不得在未询问关键词/标签库及其优先策略前正式生成标签；不得把优先关键词当作无证据事实或用于绕过允许、受限、禁止标签规则；人物身份、敏感属性、商品关键属性和低置信度结果必须人工复核。"),
...
ROLE_DEFS = [
    ("客售前后/客服角色", "客售前后高级经理", "senior-customer-service-presales-manager", "客售前后高级经理", "统筹跨渠道售前咨询、订单中服务、售后处理、商品推荐、多语言沟通、知识库、质检培训和跨部门交接。", ["咨询、售后量、响应和解决质量", "商品事实、推荐适配和转化", "订单、履约、退款退货和投诉升级", "多语言体验、知识库、质检和团队能力", "线索、售后、评价和客户声音闭环"]),
...
BUSINESS_ROLE_NAMES = {
    "senior-customer-service-presales-manager": "客售前后资深经理",
...
for role_area, folder, role_id, display, positioning, lenses in ROLE_DEFS:
    ...
    elif role_area.startswith("客售前后/"):
...
FAMILY_STEPS = {
    ...
    "customer-presales": [
        ...
        "澄清客户场景、偏好、预算、订单状态和硬约束，区分咨询、推荐、异议、投诉、退款退货和销售线索",
        "给出事实准确、文化自然且不过度承诺的答复或处理方案，明确替代、升级、补偿权限和跟进动作",
        "记录意图、解决状态、未解决原因、知识缺口、转化、退款或流失信号，按最小化原则处理个人数据",
        "输出会话、知识、质检、咨询分析、售后升级、线索交接和 VOC 闭环，并用解决质量而非单一响应速度验收",
    ],
    "creative-production": [
    ...
    "creative-tagging": [
        "生成前必须询问使用者是否已有关键词库或标签库；如有，收集文件、版本、语言、市场、来源、owner、更新时间、允许范围和禁用项",
        "继续询问是否优先参考使用者提供的关键词，并记录为强制候选、优先候选或仅供参考；未回答前可检查素材与系统，但不得正式生成标签",
        "确认贵司素材库、CMS/DAM、素材 ID、标签使用场景、搜索与效果分析消费者以及回写权限",
        "盘点并映射现有关键词/标签 taxonomy、定义、同义词、层级、互斥依赖、必填范围、敏感等级、owner 和版本；冲突项转人工",
    ...
    "customer-presales-role": [
        ...
        "建立售前、售中、售后、推荐、知识、投诉、退款退货、线索、升级和客户声音的流程、状态与责任矩阵",
        "读取咨询量、响应、解决、转化、订单、退款、投诉、流失、质检、知识命中和客户反馈证据，识别主要障碍",
        "编排咨询分析、商品推荐、多语言沟通、订单售后、FAQ 知识库、线索分级和质检培训专项 Skill",
        "在客户适配、转化、利润、库存、履约、隐私、品牌和坐席效率间做取舍并记录决策",
        "建立日周月服务节奏、会话抽检、知识更新、跨班次交接、销售与售后 SLA 和 VOC 改进闭环",
    ],
    "cashier-role": [
...
OFFICIAL_SOURCES = {
    ...
    "app-growth-promotion-specialist": [("Apple App Store Product Page", "https://developer.apple.com/app-store/product-page/"), ("Apple Product Page Optimization", "https://developer.apple.com/help/app-store-connect/create-product-page-optimization-tests/overview-of-product-page-optimization/"), ("Google Play Store Listing Guidance", "https://support.google.com/googleplay/android-developer/answer/15191715")],
...
def write_tagging_compliance_resources(base):
    (base / "assets/keyword-library-intake-template.md").write_text("""# 关键词与标签库生成前确认
...
2. 正式生成前必须询问是否存在关键词/标签库及是否优先参考给定关键词，并保存库版本、用途、owner 和优先策略；未确认前不得生成正式标签。
3. 用户提供的关键词只有在素材存在直接证据且符合允许标签库时才能被优先；优先级不能提高视觉置信度、自动创建标签或覆盖合规规则。
4. 只能从 `allowed_tags.json` 选择自动标签，不得自由创造自然语言标签。
5. `restricted_tags.json` 中任何标签必须设置 `REVIEW_REQUIRED`。
6. 请求或模型候选命中 `forbidden_tags.json` 时拒绝对应标签；若任务目的本身要求敏感推断，则整单 `COMPLIANCE_REJECTED`。
7. 不得根据外貌、肤色、服装、环境、姿态或文字推断民族、国籍、宗教、政治、健康、性取向、经济状况、社会身份或具体人物身份。
8. 不做人脸识别，不输出姓名、联系方式、证件、精确地址或精确年龄。
9. 检测到人物时默认提高风险；疑似未成年人或年龄无法确定时标记 `POSSIBLE_MINOR` 或 `UNKNOWN`，不得猜测年龄，并强制人工审核。
10. Logo、水印、品牌名和第三方文字只能标记风险。授权状态必须来自素材中台的授权编号、地域、渠道、期限和权利人字段，不能从图片猜测。
11. 每个标签必须带直接证据和置信度。默认：`>=0.85` 可自动接受，`0.60-0.85` 转人工，`<0.60` 不输出；受限或高风险标签不受自动阈值豁免。
12. 输出必须通过 `tagging-output.schema.json`，并记录 Skill、模型、标签库、素材、审核、操作人与时间版本。
...
""", encoding="utf-8")
    ...
    marker = "- `assets/decision-record-template.md`：方案比较、审批、异常、回滚和复盘记录。"
...
CATEGORIES = {
    ...
    "渠道运营/客户体验": ("客户体验", "覆盖渠道经营侧的评价、声誉和体验改进；具体客户接待、订单售后、投诉与咨询分析由客售前后负责。"),
    ...
    "精准营销/增长营销/APP推广": ("APP 增长推广", "覆盖 ASO、商店页、安装投放、深链归因、激活、留存、价值和反作弊。"),
...
CATEGORIES.update({
    ...
    "客售前后": ("客售前后", "覆盖购买前咨询、订单中服务、售后处理、多语言沟通、知识库、咨询分析、质检培训与跨部门交接。参见 [客售前后 Skill 地图](CUSTOMER_SERVICE_SKILLS_MAP.md)。"),
    "客售前后/客服角色": ("客售前后主角色", "沉淀客售前后资深经理及专属周报 Skill。"),
    "客售前后/客服角色/客售前后资深经理": ("客售前后资深经理", "统筹跨渠道售前、售中、售后服务、知识、质量、线索、投诉与改进闭环。"),
    "客售前后/客服运营": ("客服运营", "管理跨渠道、多语言售前售中售后接待、话术、权限、升级与客户声音。"),
    "客售前后/咨询分析": ("咨询分析", "基于授权会话及订单、商品、履约、退款和评价事实分析客户意图、问题、根因与改进机会。"),
...
def category_readme(relative, title, summary):
    ...
    if top == "法律政务":
        ...
        principles = ["先确认国家、渠道、语言、商品与订单事实源、服务阶段和坐席权限。", "客售前后负责购买前咨询、订单中服务、售后处理、知识和 VOC；渠道运营负责店铺经营规则，市场采购与仓储库存提供商品、供应和库存履约事实。", "不得为转化或平息投诉虚构功能、现货、折扣、送达、退款或赔付承诺；敏感语言和高风险事项必须人工复核。"]
