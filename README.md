...
- 全库同时维护[逐项语义审计](SKILL_FULL_AUDIT.md)，不以文件齐全或通用模板通过代替专业质量检查
- 所有 Skill 执行[数据与合规接口治理](DATA_SOURCE_GOVERNANCE.md)：先向使用者确认静态数据、系统事实源和已授权接口；关键数据不足时必须降级，禁止用 LLM 记忆补造实时业务结论
...
| 版本 | 日期 | 更新内容 |
|---|---|---|
| 6.5.0 | 2026-07-15 | 新增基础工具一级分类，提供多平台消息通知与分级告警、企业通用周报、复盘行动闭环；新增本地简历筛选与合规评估，按 JD 证据进行人工复核分层。 |
| 6.4.0 | 2026-07-15 | 全量语义审计扩展至 187 个 Skill 并要求十项全通过；新增 Meta、Google Ads、SEO 软文与合规社区分发、GEO 可信知识库及 llms/Schema 批量生成自检；Git 增加 commit、异常冲突与 test 污染门禁；PDP 增加图片合规检查，并显式展示库存补货与分级告警能力。 |
| 6.3.0 | 2026-07-15 | 对 183 个 Skill 启动逐项语义审计；将全部标准化 Skill 的场景示例和验收改为专业维度与交付物专属内容，并统一实时证据门禁。 |
| 6.2.0 | 2026-07-15 | 亚马逊 PDP Skill 新增当前市场 Rank 与趋势、SEO/GEO 关键词策略和上线前自检；PDP 与智能打标均在生成前强制询问关键词库及使用者优先策略。 |
