...
- 核心判断：围绕业务模块与 service provider、container binding 和生命周期、Eloquent、事务和查询边界、queue、event、schedule 和幂等、middleware、authorization、Octane 和部署逐项判断，不跳过不适用说明。
- 执行变体：标准、快速止损、数据不足、高风险审批和分阶段试点。
- 异常路径：缺数据、口径冲突、权限不足、依赖失败、部分成功、超时与回滚。
- 完成门槛：交付Laravel 业务模块架构、容器、数据和异步边界、安全、测试和部署方案，并给出验收证据、owner、截止和剩余风险。
