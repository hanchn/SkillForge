...
- 核心判断：围绕APIRouter 与业务模块、Depends 生命周期和授权、Pydantic 输入输出契约、async/sync 与阻塞依赖、异常、OpenAPI、测试和部署逐项判断，不跳过不适用说明。
- 执行变体：标准、快速止损、数据不足、高风险审批和分阶段试点。
- 异常路径：缺数据、口径冲突、权限不足、依赖失败、部分成功、超时与回滚。
- 完成门槛：交付FastAPI 包与路由架构、依赖和数据契约、运行、测试和部署方案，并给出验收证据、owner、截止和剩余风险。
