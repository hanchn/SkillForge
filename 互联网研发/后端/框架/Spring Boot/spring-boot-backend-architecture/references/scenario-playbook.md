# Spring Boot 后端资深专家场景作战手册

本手册用于选择真实场景路径，不能用作无证据套话。一个任务可命中多个场景，但必须指定主场景并说明组合原因。

## 场景 1：Spring Boot 服务新建或重构

- 触发信号：什么变化、问题或决策需要启动本 Skill。
- 先决条件：目标、范围、对象、国家/渠道/系统、时间与 owner。
- 最低证据：当前状态、历史基线、约束、事实源、权限和反证。
- 核心判断：围绕业务模块和 bean 边界、配置、profiles 和 secrets、事务、持久化和事件、Security 与接口授权、Actuator、测试和版本升级逐项判断，不跳过不适用说明。
- 执行变体：标准、快速止损、数据不足、高风险审批和分阶段试点。
- 异常路径：缺数据、口径冲突、权限不足、依赖失败、部分成功、超时与回滚。
- 完成门槛：交付Spring Boot 模块架构、事务与集成边界、生产运行和迁移方案，并给出验收证据、owner、截止和剩余风险。
