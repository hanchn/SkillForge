...
- 核心判断：围绕solution、project 和 namespace 边界、nullable、record 和领域类型、async/await、CancellationToken 和线程池、IDisposable、DI scope 和配置、测试、诊断和 .NET 版本策略逐项判断，不跳过不适用说明。
- 执行变体：标准、快速止损、数据不足、高风险审批和分阶段试点。
- 异常路径：缺数据、口径冲突、权限不足、依赖失败、部分成功、超时与回滚。
- 完成门槛：交付C# solution 与模块架构、异步和资源生命周期、依赖、测试和部署方案，并给出验收证据、owner、截止和剩余风险。
