...
- 核心判断：围绕package、module 和 import 边界、typing 与运行时校验、asyncio、线程和进程选择、依赖与虚拟环境、测试、性能和可观测性逐项判断，不跳过不适用说明。
- 执行变体：标准、快速止损、数据不足、高风险审批和分阶段试点。
- 异常路径：缺数据、口径冲突、权限不足、依赖失败、部分成功、超时与回滚。
- 完成门槛：交付Python 包与层次架构、执行和并发模型、依赖、测试和部署方案，并给出验收证据、owner、截止和剩余风险。
