...
- 核心判断：围绕crate、module 和 feature 边界、ownership、borrowing 和领域类型、Result、错误分层和恢复、async runtime、Send/Sync 和取消、unsafe 边界、测试和性能测量逐项判断，不跳过不适用说明。
- 执行变体：标准、快速止损、数据不足、高风险审批和分阶段试点。
- 异常路径：缺数据、口径冲突、权限不足、依赖失败、部分成功、超时与回滚。
- 完成门槛：交付Rust crate 与领域架构、并发和资源模型、错误、测试和部署方案，并给出验收证据、owner、截止和剩余风险。
...
## 场景 3：Rust workspace 和 crate 架构
