...
- 核心判断：围绕package 责任与依赖方向、context、goroutine 和背压、错误语义与接口最小化、配置、资源池和优雅停机、测试、profile 和发布逐项判断，不跳过不适用说明。
- 执行变体：标准、快速止损、数据不足、高风险审批和分阶段试点。
- 异常路径：缺数据、口径冲突、权限不足、依赖失败、部分成功、超时与回滚。
- 完成门槛：交付Go 包与服务架构、并发和取消模型、错误、测试和部署方案，并给出验收证据、owner、截止和剩余风险。
