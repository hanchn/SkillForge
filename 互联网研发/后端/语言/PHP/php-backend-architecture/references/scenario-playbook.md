...
- 核心判断：围绕namespace、PSR 和 Composer、请求生命周期与共享状态、领域类型、异常和校验、数据库事务、队列和缓存、静态分析、测试和运行模式逐项判断，不跳过不适用说明。
- 执行变体：标准、快速止损、数据不足、高风险审批和分阶段试点。
- 异常路径：缺数据、口径冲突、权限不足、依赖失败、部分成功、超时与回滚。
- 完成门槛：交付PHP 模块与包架构、请求和资源生命周期、依赖、测试和部署方案，并给出验收证据、owner、截止和剩余风险。
