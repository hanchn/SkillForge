...
## 场景 1：React 复杂功能和架构设计

- 触发信号：什么变化、问题或决策需要启动本 Skill。
- 先决条件：目标、范围、对象、国家/渠道/系统、时间与 owner。
- 最低证据：当前状态、历史基线、约束、事实源、权限和反证。
- 核心判断：围绕组件组合和领域边界、本地、URL、表单与服务端状态、副作用、并发渲染和错误恢复、性能、可访问性和设计系统、测试、依赖升级和渐进重构逐项判断，不跳过不适用说明。
- 执行变体：标准、快速止损、数据不足、高风险审批和分阶段试点。
- 异常路径：缺数据、口径冲突、权限不足、依赖失败、部分成功、超时与回滚。
- 完成门槛：交付React 方案与代码边界、状态、渲染和性能决策、测试、迁移和验证计划，并给出验收证据、owner、截止和剩余风险。

## 场景 2：React 性能或状态问题诊断
