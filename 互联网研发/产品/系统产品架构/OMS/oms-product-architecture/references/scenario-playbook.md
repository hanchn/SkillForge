...
- 核心判断：围绕订单来源和统一模型、价格、支付、风控和校验、拆单、合单和路由、取消、退款、退换和逆向、OMS 与 IMS/OFS/CRM/财务边界逐项判断，不跳过不适用说明。
- 执行变体：标准、快速止损、数据不足、高风险审批和分阶段试点。
- 异常路径：缺数据、口径冲突、权限不足、依赖失败、部分成功、超时与回滚。
- 完成门槛：交付OMS 能力地图、订单域模型与状态机、路由、异常和集成契约，并给出验收证据、owner、截止和剩余风险。
