...
- 核心判断：围绕端到端业务旅程、系统负责与不负责范围、主数据和 source of truth、跨系统命令、事件与状态、交接、SLA、对账、补偿和审计逐项判断，不跳过不适用说明。
- 执行变体：标准、快速止损、数据不足、高风险审批和分阶段试点。
- 异常路径：缺数据、口径冲突、权限不足、依赖失败、部分成功、超时与回滚。
- 完成门槛：交付业务能力与系统责任地图、主数据、状态与 source of truth 矩阵、跨系统流程、交接契约与异常闭环，并给出验收证据、owner、截止和剩余风险。
