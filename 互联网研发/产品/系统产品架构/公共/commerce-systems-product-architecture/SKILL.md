...
description: 为贵司 OMS、IMS、OFS、CMS、WMS、TMS、CRM、PLM 建立业务能力分工、主数据归属、系统接口和端到端协作边界。 Use when an AI needs to handle 跨系统业务流程设计, 系统职责重叠与空白治理, 新系统接入和系统边界调整; produce 业务能力与系统责任地图, 主数据、状态与 source of truth 矩阵, 跨系统流程、交接契约与异常闭环; and apply evidence, explicit boundaries, validation, and rollback instead of generic advice.
---
