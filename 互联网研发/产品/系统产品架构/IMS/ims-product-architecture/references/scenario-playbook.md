...
- 核心判断：围绕SKU、库存位置和批次、现货、在途、锁定和不可售、预占、扣减、释放和补偿、ATP/ATS 与渠道同步、IMS 与 OMS/OFS/WMS 边界逐项判断，不跳过不适用说明。
- 执行变体：标准、快速止损、数据不足、高风险审批和分阶段试点。
- 异常路径：缺数据、口径冲突、权限不足、依赖失败、部分成功、超时与回滚。
- 完成门槛：交付IMS 能力地图、库存账本和状态模型、预占、同步、对账和异常方案，并给出验收证据、owner、截止和剩余风险。
